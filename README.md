...
- **可選儲存位置**：設定頁切換影片 / 圖片要存到 **照片 App** 或 **App 資料夾**；音訊一律存 App 資料夾（照片庫放不了音訊）。存在 App 資料夾的項目可在列表直接**分享**到其他 App。
- **下載清單**：進行中與歷史，每列有影片 / 圖片**預覽縮圖**；可重試、單項刪除（連同 App 資料夾內的檔案與縮圖），或一鍵**清空**。
- **引擎線上更新**：啟動時背景更新 yt-dlp，解析失敗時自動更新重試，設定頁也可手動更新。

- **Threads（Meta）**：yt-dlp 無 extractor，故自行抓 `/embed` 頁、解析直連媒體（圖片在 `cdninstagram.com`、影片現多在 `fbcdn.net`），依儲存設定存入照片庫或 App 資料夾（`ThreadsService` + `DirectMediaSaver`，純解析邏輯在 `MediaDlerCore` 有單元測試）。純圖多圖輪播仍受限（需登入，多半只拿得到首圖）。
...
- 下載進度以位元組級回報（自訂 `URLSessionDownloadTask` delegate）；影音合併階段為估計值。
